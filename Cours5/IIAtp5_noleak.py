...
class SVMImplementation:
    ...
    def preprocess_data(self, X_train: np.ndarray, X_test: np.ndarray) -> np.ndarray:
        """
    ...
    def train_kernel_svm(
        ...
        if kernel=='precomputed':
            x_train = self.custom_kernel(X_train, X_train)
            x_test= self.custom_kernel(X_test, X_train)
        else:
            x_train = X_train
            x_test = X_test
    ...
    def compare_kernels(
        ...
        for kernel in kernels:
            best_params = self.parameter_optimization(X_train, y_train, kernel=kernel)[0]
            perf = self.train_kernel_svm(
                X_train=X_train,
                y_train=y_train,
                X_test=X_test,
                y_test=y_test,
                kernel=kernel,
                kernel_params=best_params) #, x_test, y_test, kernel)
            metrics[kernel] = perf
        return metrics
...
if __name__ == "__main__":
    ...
    # Example workflow
    X_train_scaled, X_test_scaled = svm_impl.preprocess_data(X_train, X_test)
    ...
    kernel_results = svm_impl.compare_kernels(
        ...
        y_test=y_test,
        kernels=["linear", "rbf", "poly", "precomputed"]
    )
